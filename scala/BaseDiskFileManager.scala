...
abstract class BaseDiskFileManager {
...
/*
    ...
    */
    def BaseDiskFileManager(Config conf, Logger logger) {
        this.logger = logger;
    ...
    @throws DiskFileError if any part of the filename is not able to be
                           validated.
    */
    def abstract parse_on_disk_filename(filename: String);
    ...
    */
    def abstract _process_ondisk_files(exts, results, kwargs);
    ...
    */
    def _verify_ondisk_files(results, kwargs) : Boolean = {
    ...
    */
    def _split_list(original_list, condition) {
    ...
    @return a tuple of two lists.
    */
    def _split_gt_timestamp(file_info_list, timestamp) {
    ...
    @return a tuple of two lists.
    */
    def _split_gte_timestamp(file_info_list, timestamp) {
        return this._split_list(
            file_info_list, lambda x: x["timestamp"] >= timestamp);
    }

    def get_ondisk_files(files: List[String],
                                 datadir: String,
                                 verify: Boolean=true,
                                 kwargs) {
        /**
    ...
    */
    def cleanup_ondisk_files(hsh_path,
    ...
    */
    ...
    def _hash_suffix_dir(String path,
                                 mapper,
                                 reclaim_age) {
    ...
    */
    def abstract _hash_suffix(path, int reclaim_age);
    ...
    */
    def _get_hashes(partition_path: String,
                            recalculate=null,
                            do_listdir: Boolean=false,
                            reclaim_age: Int=null) {
        ...
        if (do_listdir) {
            for (String suff : os.listdir(partition_path)) {
                if (suff.length() == 3) {
    ...
    */
    def construct_dev_path(String device) : String = {
        return os.path.join(this.devices, device);
    }

    def get_dev_path(device: String) : String = {
    ...
    */
    def get_dev_path(device: String, mount_check: Boolean) : String = {
    ...
    */
    def replication_lock(device: String) {
    ...
    def pickle_async_update(device: String,
                                    account: String,
                                    container: String,
                                    obj: String,
                                    byte[] data,
    ...
    def get_diskfile(device: String,
                             partition,
                             account: String,
                             container: String,
                             obj: String,
    ...
    def object_audit_location_generator(device_dirs=null) {
    ...
    def get_diskfile_from_audit_location(AuditLocation audit_location) {
    ...
    */
    def get_diskfile_from_hash(device: String,
                                           partition,
                                           object_hash: String,
                                           policy: StoragePolicy,
                                           kwargs) : DiskFile = {
    ...
    def _listdir(path: String) : List[String] = {
        try {
    ...
    */
    def yield_suffixes(device: String,
                               partition,
                               policy: StoragePolicy) {
        String dev_path = this.get_dev_path(device);
        if (dev_path == null || dev_path.length() == 0) {
    ...
    */
    def yield_hashes(device: String,
                             partition,
                             policy: StoragePolicy,
                             suffixes=null,
                             kwargs) {
