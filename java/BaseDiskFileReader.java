...
public class BaseDiskFileReader {
    ...
    */
    public BaseDiskFileReader(Object fp,
                              String data_file,
                              int obj_size,
                              Object etag,
                              ThreadPool threadpool,
                              int disk_chunk_size,
                              boolean keep_cache_size,
                              String device_path,
                              Logger logger,
    ...
    public boolean can_zero_copy_send() {
    ...
    @param wsockfd file descriptor (integer) of the socket out which to
                   send data
    */
    public void zero_copy_send(int wsockfd) {
    ...
    public void app_iter_range(int start, int stop) {
        if (start > -1) {
            this._fp.seek(start);
        }
    ...
    public void app_iter_ranges(ranges, content_type, boundary, size) {
        if (!ranges) {
            yield "";
        } else {
            try {
    ...
    public void _drop_cache(int fd, int offset, int length) {
        if (!this._keep_cache) {
            drop_buffer_cache(fd, offset, length);
        }
    }

    public void _quarantine(String msg) {
    ...
    public void _handle_close_quarantine() {
